...
export class Color {
    ...
    public convertStringColorToRgb(colorInString: string): void {
        ...
        const processedColorInString = colorInString.replace(shorthandRegex, (_m, r, g, b) => (r + r + g + g + b + b));

        const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(processedColorInString);
        if (result != null) {
